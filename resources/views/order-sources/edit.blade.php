<x-app>
    <x-content-header>
        <h1 class="m-0">{{ __('Edit Order Source') }}</h1>
    </x-content-header>
    ...
    <section class="content">
        <div class="row">
            <div class="col-lg-6">
                <form
                    action="{{ route('order-sources.update', $orderSource) }}"
                    method="POST"
                    novalidate
                >
                    @csrf
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input
                                    ...
                                    value="{{ old('name') ?? $orderSource->name }}"
                                />
                                @error('name')
                            ...
                            >
                                ...
                                <input
                                    ...
                                    id="parent_id"
                                    name="parent_id"
                                    value="{{ old('parent_id') ?? $orderSource->parent_id }}"
                                />
                                <input
                                    type="text"
                                    id="parent_name"
                                    ...
                                    value="{{ old('parent_name') ?? $orderSource->parent?->name }}"
                                />
                                @error('parent_name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
